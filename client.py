...
socket.connect ("tcp://35.200.147.230:%s" % port)
...
for request in range(1):
    ...
    print ("Sending request ", "...")
    socket.send_json ({'language': 'JAVA', 'code': codeJava, 'id': "main", 'testcasepresent': True, 'testcase': codePythonTestCase, 'timeout': '2','code_stdin':'1\n2\n'})
    #  Get the reply.
    message = socket.recv_json()
    print ("Received reply ",  "[", message, "]")
